...
const useLongDistanceLove = () => {
  ...
  const sendMessage = useCallback(async (message, userId) => {
    ...
    try {
      const response = await axios.post('https://long-distance-love.vercel.app/chat', {
        message,
        userId,
      });

      const botResponse = response.data.response; // The text response from the bot
      const ttsAudio = response.data.ttsAudio; 
  ...
  const uploadDocument = useCallback(async (file, userId) => {
    ...
    try {
      await axios.post('https://long-distance-love.vercel.app/upload', formData, {
        headers: {
...
export default useLongDistanceLove;