...
const useSleepSupport = () => {
  ...
  const sendMessage = useCallback(async (message, userId) => {
    ...
    try {
      const response = await axios.post('https://sleep-support-nine.vercel.app/chat', {
        message,
        userId,
      });
  ...
  const uploadDocument = useCallback(async (file, userId) => {
    ...
    try {
      await axios.post('https://sleep-support-nine.vercel.app/upload', formData, {
        headers: {
...
export default useSleepSupport;