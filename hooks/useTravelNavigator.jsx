...
const useTravelNavigator = () => {
  ...
  const sendMessage = useCallback(async (message, userId) => {
    ...
    try {
      const response = await axios.post('https://travelnavi.vercel.app/chat', {
        message,
        userId,
      });
  ...
  const uploadDocument = useCallback(async (file, userId) => {
    ...
    try {
      await axios.post('https://travelnavi.vercel.app/upload', formData, {
        headers: {
...
export default useTravelNavigator;