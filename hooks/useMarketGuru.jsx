...
const useMarketGuru = () => {
  ...
  const sendMessage = useCallback(async (message, userId) => {
    ...
    try {
      const response = await axios.post('https://market-guru-delta.vercel.app/chat', {
        message,
        userId,
      });
  ...
  const uploadDocument = useCallback(async (file, userId) => {
    ...
    try {
      await axios.post('https://market-guru-delta.vercel.app/upload', formData, {
        headers: {
