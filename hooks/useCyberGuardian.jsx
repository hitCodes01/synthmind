...
const useCyberGuardian = () => {
  ...
  const sendMessage = useCallback(async (message, userId) => {
    ...
    try {
      const response = await axios.post('https://cyberguardian-peach.vercel.app/chat', {
        message,
        userId,
      });
  ...
  const uploadDocument = useCallback(async (file, userId) => {
    ...
    try {
      await axios.post('https://cyberguardian-peach.vercel.app/upload', formData, {
        headers: {
...
export default useCyberGuardian;