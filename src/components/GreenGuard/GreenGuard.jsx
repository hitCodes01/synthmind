...
import useGreenGuard from "../../../hooks/useGreenGuard";

export const GreenGuard = ({ hidden, ...props }) => {
  const input = useRef();
  const { conversation, sendMessage, uploadDocument, isLoading, error } = useGreenGuard();
    ...
    >
      ...
      <div className="relative flex flex-col h-full w-full">
        <div className="bg-gradient-to-r from-cyan-500 via-teal-600 to-blue-900 p-4 sm:p-6 rounded-lg shadow-lg z-10 text-center mb-4">
          <h1 className="text-3xl sm:text-5xl text-white font-bold">GreenGuard EcoMate</h1>
          <p className="text-lg sm:text-xl text-white mt-2">Protect The Environment!</p>
        </div>
