...
import useFinanceWizard from "../../../hooks/useFinanceWizard";
...
export const FinanceWizard = ({ hidden }) => {
  const input = useRef();
  const { conversation, sendMessage, uploadDocument, isLoading, error } = useFinanceWizard();
    ...
    >
      ...
      <div className="relative flex flex-col h-full w-full z-10">
        <div className="bg-gradient-to-r from-blue-800 via-blue-600 to-gray-700 p-4 sm:p-6 rounded-lg shadow-lg text-center mb-4">
          <h1 className="text-3xl sm:text-5xl text-white font-bold">Finance Wizard Pro™</h1>
          <p className="text-lg sm:text-xl text-white mt-2">Manage Your Finances!</p>
        </div>
