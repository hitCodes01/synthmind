...
import useLegalEagle from "../../../hooks/useGriefGuide";
import useGriefGuide from "../../../hooks/useGriefGuide";

export const GriefGuide = ({ hidden }) => {
  const input = useRef();
  const { conversation, sendMessage, uploadDocument, isLoading, error } = useGriefGuide();
    ...
    >
      ...
      <div className="relative flex flex-col h-full w-full z-10">
        <div className="bg-gradient-to-r from-blue-500 via-green-400 to-green-700 p-4 sm:p-6 rounded-lg shadow-lg text-center mb-4">
          <h1 className="text-3xl sm:text-5xl text-white font-bold">Grief Guide</h1>
          <p className="text-lg sm:text-xl text-white mt-2">Your Mourning Buddy</p>
        </div>
