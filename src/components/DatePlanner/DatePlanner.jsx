...
export const DatePlanner = ({ hidden, ...props }) => {
  const input = useRef();
  const { conversation, sendMessage, uploadDocument, isLoading, error } = useDatePlanner();
  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-col h-full w-full">
        ...
        <div className="bg-blue-900 p-4 sm:p-6 rounded-lg shadow-lg z-10 text-center mb-4">
          <h1 className="text-3xl sm:text-5xl text-white font-bold">Date Planner Pro™</h1>
          <p className="text-lg sm:text-xl text-gray-300 mt-2">Fix Your Dates</p>
        </div>

        {/* Conversation Section */}
        <div className="relative z-10 flex flex-col flex-grow p-4 overflow-y-auto bg-gray-900 bg-opacity-80 rounded-lg shadow-lg mb-24 space-y-3">
          {conversation.map((msg, index) => (
            <div
              key={index}
              className={`mb-2 p-4 rounded-md transition-all ${
                msg.sender === "user"
