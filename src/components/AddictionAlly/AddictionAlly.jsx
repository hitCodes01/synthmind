...
import useAddictionAlly from "../../../hooks/useAddictionAlly";
...
export const AddictionAlly = ({ hidden }) => {
  const input = useRef();
  ...
  const toggleTTS = () => {
    setIsTTSEnabled(prev => {
      ...
      triggerNotification(
        newState
          ? "Text-to-Speech enabled! Responses will include audio but may take slightly longer to process. Enjoy chatting!"
  ...
  return (
    ...
    >
      ...
      <div className="fixed flex flex-col h-full w-full z-10">
        <div className="bg-gradient-to-r from-blue-500 via-green-400 to-green-700 p-2 sm:p-6 rounded-lg shadow-lg text-center mb-4">
          <h1 className="text-3xl sm:text-5xl text-white font-bold">Addiction Counsel AI™</h1>
          <p className="text-lg sm:text-xl text-white mt-2">Recover From Addiction</p>
        </div>
        ...
        {showNotification && (
          <div className="fixed bottom-20 left-1/2 transform -translate-x-1/2 bg-green-600 text-white p-3 rounded-lg shadow-lg z-30">
            {showNotification}
          </div>
        )}
      </div>
